...
class LinearChainData(Data):
    def __init__(self, inFile, vocab=None):
        ...
        #Remove features that appear fewer than MIN_COUNT times in the training data
#        MIN_COUNT = 5
#        featureCounts = np.zeros(self.vocab.GetVocabSize())
#        for (s,t) in self.featurizedSentences:
#            featureCounts += np.ravel(np.sum(s.todense(), axis=0).flatten())
#        for (s,t) in self.featurizedSentences:
#            s[:, featureCounts <= MIN_COUNT] = 0
